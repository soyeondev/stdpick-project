...
$(function() {
	...
	var pass_no = $("#pass_no").val();				//넘어온값 //넘겨줄값
	var amount = $("#amount").val();				//넘어온값 //넘겨줄값
	...
	$.ajax({url:"detailPass",data:{"pass_no":pass_no} , dataType:"JSON", async: false ,
		success:function(data) {
			pass_content = data.pass_content;
			$("#pass_content").val("구매 이용권 : "+data.pass_content);
	...
	$("#btnPayment").click(function() {
		...
		//결제 API =======================================================	
// 		alert("ajax 들어간다");
// 		$.ajax({url:"insertGetPassPayment",data:data , async:false, success:function(r) {
// 				re = Number(r);
				
// 				var product = "구매한 이용권 : " + pass_content + " - " + amount + "개";
// 				$.ajax({url:"mailSender", data:{"email":email, "cust_name":cust_name,
// 					"product":product, "resultPrice":resultPrice}, async:false,
...
// 		}});
		...
		IMP.request_pay({
		    ...
		    if ( rsp.success ) {	//결제 성공시 DB에 정보 insert하기
		    	...
		    	if(re>1) {
		    		//메세지 또는 메일 보내기 (구매확인)
			    	var product = pass_content + " : " + amount + "개";
					$.ajax({url:"mailSender", data:{"email":email, "cust_name":cust_name,
		    ...
		    window.location.href = "paymentResultView.do?msg="+msg;
...
<body>
	...
	<input type="hidden" id="pass_no" value="${pass_no}">
	<input type="hidden" id="amount" value="${amount}">
	<input type="hidden" id="totalPrice" value="${totalPrice}">


	<div class="smart-forms smart-container wrap-1">
	<h3>${cafe_name} - 이용권 결제</h3>
	<br><br>
		<div class="form-body">
			...
			<div class="section">
				<label class="field prepend-icon"> <input type="text" id="pass_content"
					class="gui-input" readonly="readonly">
			...
			<div class="section">
				<label class="field prepend-icon"> <input type="text"
					class="gui-input" value="수량 : ${amount}개"
